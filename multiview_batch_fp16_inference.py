...
# 2. Prompt Caching
# ============================================================
def build_cached_prompt(processor, view: str = "front", drive_direction: str = "forward"): # backward, forward and turning left, forward and turning right, ...
    base_text = (
        f"You are an autonomous driving safety expert analyzing this ego vehicle's {view} view video for EXTERNAL ANOMALIES that may impact safe AV operation.\n\n"
        "<think>\n"
...
def analyze_video_batch(
    model, processor, batch_items: list[tuple[Path, tuple, str]], max_tokens: int
) -> list[str]:
    """
    Process a batch of videos in a single forward pass.
    batch_items: list of (video_path, (image_inputs, video_inputs), view) tuples.
    ...
    for video_path, (image_inputs, video_inputs), view in batch_items:
        prompt_text = build_cached_prompt(processor, view)
        content = [
...
def extract_view(filename: str) -> str:
    "This will return front if filename does not contain view informations"
    ...
    for batch_start_idx in range(0, len(video_files), args.batch_size):
        ...
        for vp in batch_paths:
            try:
                ...
                view = extract_view(vp.name)
                batch_items.append((vp, prefetched, view))
        ...
        try:
            raw_outputs = analyze_video_batch(
                model, processor, batch_items, args.max_tokens
            )
        except Exception as e:
            # If the whole batch fails, record errors for all items
            for vp, _, _ in batch_items:
                counts["Error"] += 1
                results.append({
                    "file": vp.name,
                    "result": "Error",
        ...
        # --- Process results ---
        for idx, ((vp, _, _), raw) in enumerate(zip(batch_items, raw_outputs)):
